...
preprocess_data <- function(var_to_predict, train_df, val_df,
                            ...
                            prop_switch_y,prop_noise_x,q_noisy_var,dataset_actual,noise_level) {
  ...
  if (prop_noise_x > 0) {
    ...
    else if (dataset_actual == "Churn"){
      df <- read.csv("./data/customer_churn.csv")
      tree <- rpart(churn~., data = df, control=rpart.control(minsplit=2, minbucket=1, maxdepth=10, cp=0, xval=0))
    }
    else {
      stop("La data solicitada no se encuentra cargada")
    }
...
rep_val_estimate <- function(var_to_predict, tree_control, data_df, prop_val, reps, preprocess_control) {
  ...
  for (r in 1:reps) {
    ...
    processed_dfs <- preprocess_data(var_to_predict, train_df,
                                     ...
                                     preprocess_control$noise_level)
