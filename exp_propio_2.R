...
run_experiment <- function(datasets_to_pred, filepath) {
  ...
  for (dtp in datasets_to_pred) {
    for (prop_switch_x in seq(0.5, 1, 0.1)) {
      print(c(dtp$dataset_name, prop_switch_x))
...
plot_exp_results <- function(filename_exp_results, filename_plot, width, height) {
  ...
  data_for_plot <- exp_results %>%
    group_by(dataset_name, prop_switch_x, maxdepth) %>%
  ...
  g <- ggplot(data_for_plot, aes(x=maxdepth, y=mean_auc)) +
    ...
    facet_grid(dataset_name ~ prop_switch_x) +
...
plot_exp_results( "./outputs/tables/exp_propio_2.txt", "./outputs/plots/exp_propio_2.jpg", width=15, height=8)
